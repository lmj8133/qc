...
| size | 無畫面 | 有畫面 | 瓶頸 |
|---|---|---|---|
| 512 | **30.1 FPS** | 25.7 FPS（並排）／29.5（`--depth-only`） | 顯示頻寬 |
| **384** ⬅ 展示建議 | 30.0 FPS | **30.0 FPS（並排）** | **相機**（已跑滿 30fps 上限） |
...
**此平台最終要用於避障**，這會改變後續的優化取捨：

- **延遲比幀率重要。** 避障關心的是「從鏡頭進光到做出決策」的時間，不是每秒幾張。
  目前單執行緒序列架構的單幀延遲最短（約 33 ms），這對避障是優點。
- **不要做 double-buffer。** 它能把 640px 從 22.4 推到約 32 FPS，但代價是每幀多等一輪
  —— 吞吐量上升、單幀延遲上升。看即時影像無感，但控制迴路會拿到更舊的深度圖。
  詳細理由見 `yolo-depth/pipeline/README.md`。
- **後續若要優化，量的是端到端延遲，不是 FPS。** 對避障而言，「更快但更晚到」是退步。

### 尚未驗證

- **精度僅比對 2 張圖** —— 未跑 NYU val 654 張完整 Delta1/RMSE，無法對照官方 0.882 / 0.414m
- **640px 未達 30 FPS** —— 各階段嚴格序列相加。可用 double-buffer 或把 resize 移到 GPU 解決，
  但**基於上述避障考量，目前刻意不做**。512px 已是相機上限（30 fps），做了也無增益
- **端到端延遲未量測** —— 目前只量了各階段耗時與吞吐量。避障要的「進光→決策」延遲
  還包含相機曝光、USB 傳輸、驅動緩衝，這些都尚未納入量測
